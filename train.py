...
parser.add_argument('--sz_fc', default = 512, type = float,
    help = 'size of fc'
)
...
parser.add_argument('--N', default = '5', type = int,
    help = 'Number of generated point'
)
parser.add_argument('--beta', default = '48', type = float,
    help = 'gen_loss scale'
)
parser.add_argument('--delta', default = '0.11', type = float,
    help = 'gen_loss margin'
)
parser.add_argument('--r', default = '1.0', type = float,
    help = 'generation strength'
)
parser.add_argument('--lam', default = '0.23', type = float,
    help = 'the scale of gen_loss and basic loss'
)
parser.add_argument('--gen_mrg', default = '0.05', type = float,
    help = 'how to choose data point'
)
parser.add_argument('--scale', default = '32', type = float,
    help = 'the scale parameter of gen loss, which is a number lager than zero'
)
parser.add_argument('--ratio_gen', default = '0.9', type = float,
    help = 'the ratio of gen loss and metric loss in get_gen_loss2, which is a number between 0 to 1'
)
parser.add_argument('--ratio_m', default = '0.9', type = float,
    help = 'the ratio of gen loss and metric loss in get_gen_loss2, which is a number between 0 to 1'
)
...
def main():
  ...
  LOG_DIR = args.LOG_DIR + '/logs_{}/{}_embedding/{}_batchsize/{}_{}_alpha{}_beta{}_delta{}_lam{}_mrg{}_{}_lr{}_N{}_r{}_epochs{}_fc{}_genmrg{}_scale{}_ratio_gen{}_ratio_m{}'.format(args.dataset,
                                                                                                   args.sz_embedding,
                                                                                                   args.sz_batch,
  ...
  data_root = os.path.join('/media/', 'lzt')
  ...
  if args.loss == 'Proxy_Anchor':
      criterion = losses.Proxy_Anchor(nb_classes = nb_classes, sz_embed = args.sz_embedding, sz_fc = args.sz_fc, mrg = args.mrg, alpha = args.alpha, N = args.N, beta = args.beta, delta= args.delta, r = args.r,
                                      lam = args.lam, gen_mrg=args.gen_mrg, scale=args.scale, ratio_gen=args.ratio_gen, ratio_m = args.ratio_m).cuda()
  ...
  for epoch in range(0, args.nb_epochs):
      ...
      for batch_idx, (x, y) in pbar:                         
          ...
          a = loss.data
          losses_per_epoch.append(loss.data.squeeze().cpu().numpy())
          opt.step()
